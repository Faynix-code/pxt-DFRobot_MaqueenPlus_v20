...
namespace maqueenPlusV2 {
  ...
  export function moveMotor(
    ...
    if (bluetoothEnabled) {
      bluetooth.uartWriteString(
        `Moteur: ${motor}, Vitesse: ${speed}, Direction: ${direction}`
      );
    } else {
      let pin =
        motor === MyEnumMotor.LeftMotor ? DigitalPin.P13 : DigitalPin.P14;
      let dirPin =
        motor === MyEnumMotor.LeftMotor ? DigitalPin.P15 : DigitalPin.P16;
  ...
  export function showColor(pin: DigitalPin, rgb: number) {
    ...
    let b = (rgb & 0xff) * (_brightness / 255);
    for (let i = 0; i < 16 * 3; i++) {
      if (i % 3 == 0) neopixel_buf[i] = Math.round(g);
      if (i % 3 == 1) neopixel_buf[i] = Math.round(r);
      if (i % 3 == 2) neopixel_buf[i] = Math.round(b);
    }
    ws2812b.sendBuffer(neopixel_buf, pin);
  }
  ...
  export function patrolling(patrol: Patrolling) {
    let allBuffer = pins.createBuffer(2);
    if (patrol == Patrolling.ON) allBuffer[1] = 0x04 | 0x01;
    else allBuffer[1] = 0x08;
    allBuffer[0] = 60;
    pins.i2cWriteBuffer(I2CADDR, allBuffer);
  }
  ...
  export function readLightIntensity(type: DirectionType2): number {
    ...
    if (type == DirectionType2.Left) return (allBuffer[0] << 8) | allBuffer[1];
    else return (allBuffer[2] << 8) | allBuffer[3];
  }
  ...
  //% group="V3"
  //% advanced=true
  export function pidControlDistance(
    dir: SpeedDirection,
    distance: number,
    interruption: MyInterruption
  ) {
    let speed = 2;
    let allBuffer = pins.createBuffer(2);
    if (distance >= 6000) distance = 60000;
    allBuffer[0] = 64;
    allBuffer[1] = dir;
    pins.i2cWriteBuffer(I2CADDR, allBuffer);
    allBuffer[0] = 85;
    ...
    pins.i2cWriteBuffer(I2CADDR, allBuffer);
    allBuffer[0] = 65;
    allBuffer[1] = distance >> 8;
    pins.i2cWriteBuffer(I2CADDR, allBuffer);
    allBuffer[0] = 66;
    allBuffer[1] = distance;
    pins.i2cWriteBuffer(I2CADDR, allBuffer);
    allBuffer[0] = 60;
    allBuffer[1] = 0x04 | 0x02;
    pins.i2cWriteBuffer(I2CADDR, allBuffer);
  ...
  export function readRealTimeSpeed(type: DirectionType2): number {
    ...
    if (type == DirectionType2.Left) return allBuffer[0] / 5;
    else return allBuffer[1] / 5;
  }
